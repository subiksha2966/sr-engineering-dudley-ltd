...
import { ArrowRight, CheckCircle, Phone, Flame, Zap, Droplets, ClipboardCheck, Star, ChevronRight } from 'lucide-react';
import { useScrollReveal } from '../hooks/useScrollReveal';

interface HomeProps { darkMode: boolean }

const services = [
  { icon: Flame,         label: 'Welding & Fabrication', desc: 'Precision MIG, TIG & arc welding for structural and bespoke metalwork.' },
  { icon: ClipboardCheck, label: 'PAT Testing',          desc: 'Full portable appliance testing with certification and detailed reports.' },
  { icon: Droplets,      label: 'Plumbing Solutions',    desc: 'Residential and commercial plumbing, boiler maintenance, and installs.' },
  { icon: Zap,           label: 'Electrical Services',   desc: 'NICEIC-registered electricians for rewires, fuse boards, and fit-outs.' },
];
...
const highlights = ['Plumbing Solutions', 'Electrical Services', 'PAT Testing', 'Welding & Fabrication'];

const testimonials = [
  { name: 'James Holloway', company: 'Holloway Construction Ltd', rating: 5, review: 'Outstanding structural welding work delivered on a tight deadline. Professional team, excellent quality.' },
  { name: 'Sarah Mitchell', company: 'Mitchell & Sons Property', rating: 5, review: 'PAT testing across our entire portfolio — efficient, thorough, and the reports were exactly what our insurers required.' },
  { name: 'David Kaur', company: 'DK Industrial Services', rating: 5, review: 'Emergency plumbing response within the hour. Had us back running by end of day. Exceptional service.' },
];
...
export default function Home({ darkMode }: HomeProps) {
  ...
  useEffect(() => {
    const targets = heroRef.current?.querySelectorAll<HTMLElement>('.hero-reveal');
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center min-h-screen lg:min-h-0 lg:py-32">
            <div>
              ...
              <h1 className="hero-reveal opacity-0 text-4xl sm:text-5xl lg:text-6xl font-black text-white leading-[1.06] tracking-tight mb-6">
                ...
                <span className="text-blue-400">for a Solid</span>{' '}
                Future
              </h1>

              <p className="hero-reveal opacity-0 text-lg text-gray-300 leading-relaxed mb-8 max-w-xl">
                SR Engineering Dudley LTD delivers expert plumbing, electrical services, 
                PAT testing, and welding &amp; fabrication — built on quality, safety, and reliability.
              </p>

              <div className="hero-reveal opacity-0 grid grid-cols-2 gap-2.5 mb-10 max-w-sm">
                {highlights.map((h) => (
                  <div key={h} className="flex items-center gap-2 text-gray-200 text-sm font-medium">
            ...
            <div className="hero-reveal opacity-0 hidden lg:grid grid-cols-2 gap-4">
              {stats.map((s) => (
                <div key={s.label} className="bg-white/10 backdrop-blur-md border border-white/15 rounded-2xl p-6 text-center hover:bg-white/15 transition-colors">
      ...
      <section className={`py-24 lg:py-32 ${darkMode ? 'bg-gray-950' : 'bg-gray-50'}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <RevealSection className="text-center max-w-2xl mx-auto mb-14">
            <div className="reveal opacity-0 inline-flex items-center gap-2 bg-blue-100 text-blue-700 text-sm font-semibold px-4 py-1.5 rounded-full mb-4">Our Services</div>
            <h2 className={`reveal opacity-0 text-3xl sm:text-4xl lg:text-5xl font-black tracking-tight leading-tight ${darkMode ? 'text-white' : 'text-gray-900'}`}>
              Engineering Solutions <span className="text-blue-600">You Can Trust</span>
            </h2>
          </RevealSection>

          <RevealSection className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {services.map((s) => (
              <div
                key={s.label}
                className={`reveal opacity-0 group flex flex-col items-start gap-4 p-6 rounded-2xl border transition-all duration-300 hover:-translate-y-2 hover:shadow-xl ${
                  darkMode ? 'bg-gray-800 border-gray-700 hover:border-blue-500/50' : 'bg-white border-gray-100 shadow-md hover:border-blue-200'
                }`}
              >
                ...
                <div>
                  <h3 className={`font-black text-base mb-2 ${darkMode ? 'text-white' : 'text-gray-900'}`}>{s.label}</h3>
                  <p className={`text-sm leading-relaxed ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>{s.desc}</p>
                </div>
                <NavLink
                  to="/services"
                  className={`mt-auto flex items-center gap-1.5 text-sm font-bold transition-colors ${darkMode ? 'text-blue-400 hover:text-blue-300' : 'text-blue-600 hover:text-blue-700'}`}
                >
      ...
      <section className={`py-24 lg:py-32 ${darkMode ? 'bg-gray-900' : 'bg-white'}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            ...
            <RevealSection>
              <div className="reveal opacity-0 inline-flex items-center gap-2 bg-blue-100 text-blue-700 text-sm font-semibold px-4 py-1.5 rounded-full mb-5">Why Choose SR Engineering</div>
              <h2 className={`reveal opacity-0 text-3xl sm:text-4xl font-black tracking-tight leading-tight mb-5 ${darkMode ? 'text-white' : 'text-gray-900'}`}>
                Committed to <span className="text-blue-600">Excellence</span> in Every Project
              </h2>
              <p className={`reveal opacity-0 text-base leading-relaxed mb-8 ${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                Based in Dudley, we combine 15+ years of engineering expertise with a genuine commitment
                to quality, safety, and customer satisfaction on every project — no matter the scale.
              </p>
              <div className="reveal opacity-0 grid grid-cols-2 gap-3 mb-8">
                {['NICEIC Registered', 'Fully Insured', 'City & Guilds Qualified', 'Free Quotes', '24/7 Emergency Cover', '15+ Years Experience'].map((item) => (
              ...
              <div className="reveal opacity-0 flex gap-4">
                <NavLink to="/about" className={`inline-flex items-center gap-2 font-bold px-6 py-3 rounded-xl border-2 transition-all ${darkMode ? 'border-gray-600 text-white hover:border-blue-500' : 'border-gray-200 text-gray-800 hover:border-blue-500 hover:text-blue-600'}`}>
                  About Us <ArrowRight size={16} />
                </NavLink>
                <NavLink to="/contact" className="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white font-bold px-6 py-3 rounded-xl transition-all shadow-md">
                  Get a Quote
                </NavLink>
              </div>
            </RevealSection>
      ...
      <section className={`py-24 lg:py-32 ${darkMode ? 'bg-gray-950' : 'bg-gray-50'}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <RevealSection className="text-center max-w-xl mx-auto mb-14">
            <div className="reveal opacity-0 inline-flex items-center gap-2 bg-blue-100 text-blue-700 text-sm font-semibold px-4 py-1.5 rounded-full mb-4">Client Reviews</div>
            <h2 className={`reveal opacity-0 text-3xl sm:text-4xl font-black tracking-tight ${darkMode ? 'text-white' : 'text-gray-900'}`}>
              What Our <span className="text-blue-600">Clients Say</span>
            </h2>
          </RevealSection>

          <RevealSection className="grid sm:grid-cols-3 gap-6">
            {testimonials.map((t) => (
              <div
                key={t.name}
              ...
              >
                ...
                <p className={`text-sm leading-relaxed mb-5 ${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>"{t.review}"</p>
                <div className="flex items-center gap-3 pt-4 border-t border-gray-100 dark:border-gray-700">
                  <div className="w-9 h-9 bg-blue-600 rounded-full flex items-center justify-center flex-shrink-0">
                    <span className="text-white text-xs font-black">{t.name.split(' ').map(n => n[0]).join('')}</span>
                  </div>
                  <div>
      ...
      <section className="relative py-24 overflow-hidden">
        ...
        <div className="relative z-10 max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            ...
            >
              Request a Consultation <ArrowRight size={18} className="group-hover:translate-x-1 transition-transform" />
            </NavLink>
            <a
              href="tel:+447393264576"
