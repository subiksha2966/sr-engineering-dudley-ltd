...
export default function Navbar({ darkMode, toggleDarkMode }: NavbarProps) {
  ...
  const solidBg = darkMode ? 'bg-gray-900/97 shadow-lg shadow-black/30' : 'bg-white/97 shadow-lg shadow-gray-200/60';
  const transparentBg = 'bg-transparent';
  const useSolid = !isHome || scrolled;

  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 backdrop-blur-md ${useSolid ? solidBg : transparentBg}`}
    >
          ...
          <NavLink to="/" className="flex items-center gap-3 group flex-shrink-0">
            <img
              ...
              className={`transition-all duration-300 object-contain ${
                useSolid ? 'h-11 w-11' : 'h-14 w-14'
              } ${!useSolid && !darkMode ? 'brightness-0 invert' : ''}`}
            />
            <div className="hidden sm:block">
              <p className={`font-black text-base leading-tight tracking-tight transition-colors ${
                useSolid ? (darkMode ? 'text-white' : 'text-[#1a2e5c]') : 'text-white'
              }`}>
                SR Engineering
              </p>
              <p className={`text-xs font-semibold leading-none transition-colors ${
                useSolid ? (darkMode ? 'text-blue-400' : 'text-blue-600') : 'text-blue-300'
              }`}>
                Dudley LTD
              </p>
            </div>
